...
public class Update {
	public static void main(String[] args) {
		...
		UpdateResult updateResult = users.updateOne(filter, updateOperation);
		System.out.println("=> Updating the doc with {\"student_id\":10000}. Adding comment.");
		System.out.println(users.find(filter).first().toJson(prettyPrint));
		...
		updateResult = users.updateOne(filter, updateOperation, options);
		System.out.println(
		...
		updateResult = users.updateMany(filter, updateOperation);
		...
		Document oldVersion = users.findOneAndUpdate(filter, updates);
		...
		Document newVersion = users.findOneAndUpdate(filter, updates, optionAfter);
